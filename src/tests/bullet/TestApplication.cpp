...
public:
		...
		: m_ownerWorld (ownerWorld)
	{
		...
		for ( i=0; i<NUM_LEGS; i++)
		{
			...
			hingeC->setLimit(btScalar(-0.75 * M_PI), btScalar(M_PI));
...
{
	TestApplication* testApplication = (TestApplication*)world->getWorldUserInfo();

	testApplication->setMotorTargets(timeStep);
...
//	m_fMuscleStrength = 0.05f;
	...
	btVector3 startOffset(1,0.5,10);
	spawnTestRig(startOffset, false);
	startOffset.setValue(-2,0.5,10);
...
void TestApplication::spawnTestRig(const btVector3& startOffset, bool bFixed)
{
...
void TestApplication::setMotorTargets(btScalar deltaTime)
{
	...
	my_vect current, expected;
	for (int i=0; i<2*NUM_LEGS; i++)
		{
			
		btHingeConstraint* hingeC = static_cast<btHingeConstraint*>(m_rigs[1]->GetJoints()[i]);
		btScalar fCurAngle      = hingeC->getHingeAngle();
			
		//btScalar fTargetPercent = (int(m_Time / 1000) % int(m_fCyclePeriod)) / m_fCyclePeriod;
		btScalar fTargetPercent = ((int(m_Time / 1000) % int(m_fCyclePeriod)) > int(m_fCyclePeriod / 2 )) ? 1.75 : 0.25;
		btScalar fTargetAngle   = 0.5 * (1 + sin(2 * M_PI * fTargetPercent));
		if(previousTarget_ != fTargetPercent)
		{
			previousTarget_ = fTargetPercent;
			pid_.reset();
		}
		expected(i) = fTargetAngle;
		current(i) = fCurAngle;
		}
		my_vect vDesiredAngularVel = pid_(m_Time, current, expected );

	for (int i=0; i<2*NUM_LEGS; i++)
		{
			
		btHingeConstraint* hingeC = static_cast<btHingeConstraint*>(m_rigs[1]->GetJoints()[i]);		
		hingeC->enableAngularMotor(true, vDesiredAngularVel(i), m_fMuscleStrength);

		
		btHingeConstraint* hingeC2 = static_cast<btHingeConstraint*>(m_rigs[0]->GetJoints()[i]);		
		hingeC2->enableAngularMotor(true, vDesiredAngularVel(i), m_fMuscleStrength);
		}
	//
...
void TestApplication::clientMoveAndDisplay()
{
...
void TestApplication::displayCallback()
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); 
...
void TestApplication::exitPhysics()
{
